...
async function checkOrCreateUserProfile({ phone, name }) {
  ...
  try {
    ...
    return {
      profileId: response.data.profileId || response.data.post_id,  // Ensure profileId is always present
      ...response.data
    };
  } catch (error) {
    console.error('Error checking or creating user profile:', error.response ? error.response.data : error.message);
...
async function processMessage({ data, device } = {}) {
  ...
  const name = chat.contact.displayName || 'Unknown';

  logToFile(`[info] Received message from ${name} (${phone})`);

  let userProfile;
  try {
    userProfile = await checkOrCreateUserProfile({ phone, name });
  } catch (error) {
    logToFile(`[error] Error checking or creating user profile: ${error.message}`);
    return await reply({ message: 'There was an error processing your profile. Please try again later.' });
  }

  const profileId = userProfile.profileId;
  logToFile(`[info] User profile found or created: ${JSON.stringify(userProfile)}`);

  if (!profileId) {
    logToFile(`[error] Profile ID is not defined`);
    return await reply({ message: 'There was an error processing your profile. Please try again later.' });
  }
  ...
  if (body === '2') {
    await reply({ message: 'Fetching your purchase history...' });

    try {
      // Fetch the user's receipts from WordPress
      const response = await axios.get('http://whatsapp-chatbot.local/wp-json/custom/v1/receipts', {
        params: { profile_id: profileId },
        headers: {
          'Authorization': `Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwOi8vd2hhdHNhcHAtY2hhdGJvdC5sb2NhbCIsImlhdCI6MTcyNDEyMzYwMSwibmJmIjoxNzI0MTIzNjAxLCJleHAiOjE3MjQ3Mjg0MDEsImRhdGEiOnsidXNlciI6eyJpZCI6IjEifX19.VyDaJiI8ADhwkLlsr5NojT1QOcetGuXuEVhfwba4KVw`, // Replace with your actual JWT token
        }
      });
      ...
      if (receipts.length === 0) {
        return await reply({
          message: 'No purchase history found. You have not submitted any receipts yet.'
        });
      }

      // Format the receipts for display
      const receiptMessages = receipts.map(receipt => {
        const date = new Date(receipt.date_uploaded).toLocaleDateString(); // Format the date
        const imageUrl = receipt.receipt_image; // Get the receipt image URL
        return `🛒 Purchase on ${date}: Receipt URL - ${imageUrl}`;
      }).join('\n');

      await new Promise(resolve => setTimeout(resolve, 1000));

      return await reply({
        message: `Your purchase history is as follows:\n\n${receiptMessages}\n\nFor more details, visit your account dashboard.`
      });
    } catch (error) {
      logToFile(`Error fetching purchase history: ${error.message}`);
      return await reply({ message: 'There was an error fetching your purchase history. Please try again later.' });
    }
  }

  if (body === '3') {
    await reply({ message: 'Fetching your loyalty points and rewards...' });

    try {
      const response = await axios.get('http://whatsapp-chatbot.local/wp-json/custom/v1/user-profile', {
        params: { profile_id: profileId },
        headers: {
      ...
      return await reply({
        message: `Here are your current loyalty points and rewards:
  
        ⭐️ Loyalty Points: ${loyaltyPoints} points
        🎁 Available Rewards:
        - 10% off on your next purchase (Redeemable at 1000 points)
        - Free Shipping on orders over $50 (Redeemable at 500 points)
  
        Type *redeem* followed by the reward name to claim your reward.`
      });
    } catch (error) {
      logToFile(`Error fetching loyalty points: ${error.message}`);
      return await reply({ message: 'There was an error fetching your loyalty points. Please try again later.' });
    }
  }

  if (body === '4') {
    await reply({ message: 'Connecting you to a support agent...' });

    await new Promise(resolve => setTimeout(resolve, 1000));

    return await reply({ message: 'Please send an email to zinminkhant.alfred@gmail.com for further assistance.' });
  }
    ...
    if (chatState.expectingImage) {
      ...
      if (data.media && data.media.id) {
        ...
        try {
          ...
          const imageUrl = await uploadImageToWordPress(filePath, profileId);

          fs.unlinkSync(filePath);

          return await reply({ message: `Thank you for uploading the image. Your receipt has been uploaded successfully.` });
        } catch (error) {
...
async function uploadImageToWordPress(filePath, profileId) {
  ...
  formData.append('profile_id', profileId); // Associate image with profile
  ...
  try {
    ...
    logToFile(`Error uploading image to WordPress: ${error.message}`);
